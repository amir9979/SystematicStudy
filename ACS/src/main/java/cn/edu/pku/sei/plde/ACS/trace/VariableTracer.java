...
//        List<MethodInfo> methodInfos = _suspicious.getMethodInfo(_srcPath);
    ...
    private String traceShell(String testClassname, String classname, String functionname, String testClasspath, List<VariableInfo> vars, List<MethodInfo> methods, int errorLine) throws IOException{
        String tracePath = System.getProperty("user.dir") + "/lib/RunTestAgent.jar";//PathUtils.getAgentPath();
	...
	private void spreadForLoop(){
        ...
        try {
            ShellUtils.shellRun(Arrays.asList("javac -Xlint:unchecked -source " + ShellUtils.JAVA_VERSION + " -target " + ShellUtils.JAVA_VERSION + " -cp "+ buildClasspath(Arrays.asList(PathUtils.getJunitPath(),_testClasspath,_classpath)) +" -d "+_testClasspath+" "+ sourceFile.getAbsolutePath()));
        } catch (IOException e){
            e.printStackTrace();
        }
    }
