...
public class MethodOneFixer {
        ...
        for (String patchString: patch._patchString){
            ...
            for (int patchLine: patch._patchLines){
                ...
                try {
                    ...
                    ShellUtils.shellRun(Arrays.asList("javac -Xlint:unchecked -source " + ShellUtils.JAVA_VERSION + " -target " + ShellUtils.JAVA_VERSION + " -cp "+ buildClasspath(Arrays.asList(PathUtils.getJunitPath())) +" -d "+_classpath+" "+ targetJavaFile.getAbsolutePath()));
                }
                catch (IOException e){
    ...
    public int fix(){
        ...
        for (File javaFile: tobeCompile){
            try {
                ShellUtils.shellRun(Arrays.asList("javac -Xlint:unchecked -source " + ShellUtils.JAVA_VERSION + " -target " + ShellUtils.JAVA_VERSION + " -cp "+ buildClasspath(Arrays.asList(PathUtils.getJunitPath())) +" -d "+_classpath+" "+ javaFile.getAbsolutePath()));
            }
            catch (IOException e){
                return -1;
            }
