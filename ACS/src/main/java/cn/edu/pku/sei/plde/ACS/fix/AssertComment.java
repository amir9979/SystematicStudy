...
public class AssertComment {
    ...
    private void make(){
        try {
            ShellUtils.shellRun(Arrays.asList("javac -Xlint:unchecked -source " + ShellUtils.JAVA_VERSION + " -target " + ShellUtils.JAVA_VERSION + " -cp "+ buildClasspath(Arrays.asList(PathUtils.getJunitPath())) +" -d "+ testClassPath+" "+ targetFile.getAbsolutePath()));
        } catch (IOException e){
            e.printStackTrace();
        }
    }
