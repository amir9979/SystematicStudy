...
import cn.edu.pku.sei.plde.ACS.utils.AssertUtils;
...
import cn.edu.pku.sei.plde.ACS.utils.SourceUtils;
...
public class Asserts {
    ...
    private List<Integer> getErrorAssertLine(){
        ...
        while (true){
            ...
            try{
                ...
                ShellUtils.shellRun(Arrays.asList("javac -Xlint:unchecked -source " + ShellUtils.JAVA_VERSION + " -target " + ShellUtils.JAVA_VERSION + " -cp "+ buildClasspath(Arrays.asList(PathUtils.getJunitPath(),_testClasspath,_classpath)) +" -d "+_testClasspath+" "+ tempJavaFile.getAbsolutePath()));
            }
    ...
    public String getTrueTestFile(List<Integer> errorAssertLines){
        ...
        try {
            ...
            ShellUtils.shellRun(Arrays.asList("javac -Xlint:unchecked -source " + ShellUtils.JAVA_VERSION + " -target " + ShellUtils.JAVA_VERSION + " -cp "+ buildClasspath(Arrays.asList(PathUtils.getJunitPath(),_testClasspath,_classpath)) +" "+ tempJavaFile.getAbsolutePath()));
        } catch (IOException e){
            e.printStackTrace();
            return "";
        }
