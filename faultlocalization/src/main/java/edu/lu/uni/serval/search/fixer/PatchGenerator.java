...
public class PatchGenerator {
	...
	public Patch generatePatchForTesting(String suspiciousMethodBodyCode, String suspiciousClassName) {
		Patch patch = new Patch();
		patch.patchMethodCode = generatePatchMethodBody("public class T {\n" + suspiciousMethodBodyCode + "}\n", "public class T {\n" + FileHelper.readFile("logs/patch.txt") + "}\n");
	...
	private List<String> fixSuspiciousVariableDeclarationStatement(List<ITree> stmtCandidates, ITree suspStmtTree, String methodCode, String suspStmtCode, String returnType, String exceptionStr) {
		...
		for (ITree stmt : stmtCandidates) {
			...
			if (stmt.getType() == 21) {// Expression statement . Assignment.
				...
				if (stmt.getChildren().get(0).getType() != 7) continue; //Assignment
				...
				if (buggyExpPosition == 0) {
					...
					if (suspVars.size() <= 1) {
						String patchCode = suspStmtCode.substring(0, buggyExpPosition - suspStmtTree.getPos()) + newExp;
						if (!patchCode.equals(suspStmtCode)) {
							patchList.add(patchCode);
						}
						continue;
					}
					...
					if (variables.size() == 1 && suspVars.size() == 2) {
						...
						if (!suspVar.equals(var)) {
							...
							if (s > 0) {
								...
								String patchCode = suspStmtCode.substring(0, buggyExpPosition - suspStmtTree.getPos()) + patchExp;
								if (!patchCode.equals(suspStmtCode)) {
									patchList.add(patchCode);
								}
							}
						...
						// FIXME rename more variables.String patchCode = suspStmtCode.substring(0, buggyExpPosition - suspStmtTree.getPos()) + newExp;
						String patchCode = suspStmtCode.substring(0, buggyExpPosition - suspStmtTree.getPos()) + newExp;
						if (!patchCode.equals(suspStmtCode)) {
							patchList.add(patchCode);
						}
...
//						}
				...
				if (isType1 && isType2 && !sameType(dataTypeStr, dataTypeStrCandidate)) {
					...
					String newExp = methodCode.substring(newExpPosition, stmt.getPos() + stmt.getLength());
					if (buggyExpPosition == 0) {
						String patchCode = suspStmtCode.substring(0, suspStmtCode.indexOf(";")) + " = " + newExp;
						if (!patchCode.equals(suspStmtCode)) {
							patchList.add(patchCode);
						}
					} else {
					...
					if (variables.size() == 2 && suspVars.size() == 1) {
						...
						if (!suspVar.equals(var)) {
							...
							if (s > 0) {
								...
								patchExp += newExp.substring(varPositions.get(s - 1) + varLength - newExpPosition);
								if (!suspStmtCode.equals(patchExp)) {
									patchList.add(patchExp);
								}
							} else {
								if (!suspStmtCode.equals(newExp)) {
...
//						}
				...
				if (buggyExpType == 7 && candExpType == 7) { // Assignment
					...
					if (suspVars.size() == 0 || candVars.size() == 0) {
						String patchCode = suspStmtCode.substring(0, buggyRightExp.getPos() - suspStmtPos) + candRightExpStr + ";";
						if (!suspStmtCode.equals(patchCode)) {
							patchList.add(patchCode);
						}
						continue;
					}
					if (candVars.size() == 1 && suspVars.size() == 1) {
						...
						if (!suspVar.equals(var)) {
							...
							if (s > 0) {
								...
								String patchCode = suspStmtCode.substring(0,buggyRightExp.getPos() - suspStmtPos) + patchExp + ";";
								if (!suspStmtCode.equals(patchCode)) {
									patchList.add(patchCode);
								}
							}
...
//						}
					...
					if (suspVars.size() == 0 || candVars.size() == 0) {
						String patchCode = methodCode.substring(startPos, endPos);
						if (!suspStmtCode.equals(patchCode)) {
							patchList.add(patchCode);
						}
						continue;
...
//						}
	...
	private List<String> fixSuspiciousReturnStatement(List<ITree> stmtCandidates, ITree suspStmtTree, String methodCode, String suspStmtCode, String buggyReturnType, String exceptionStr, String newReturnType) {
		...
		for (ITree stmt : stmtCandidates) {
			...
			ITree previousStmt = identifyPreviousStmt(stmt);
			if (previousStmt != null && previousStmt.getType() == 25) {
				// TODO Not just previous one, IF related to the ReturnStatement.
...
//		if ("float".equalsIgnoreCase(returnType)) return true;
	...
	private Map<String, List<Integer>> readParameterPositions(String method, Map<String, String> parameters) {
		Map<String, List<Integer>> parameterPositions = new HashMap<>();
		ITree methodTree = new ASTGenerator().generateTreeForCodeFragment(method, TokenType.EXP_JDT);
		List<ITree> children = methodTree.getChildren();
		boolean isBodyCode = false;
		for (ITree child : children) {
			if (isBodyCode || Checker.isStatement(child.getType())) {
				isBodyCode = true;
				travelSubTrees(child.getChildren(), parameterPositions, parameters);
			}
		}
		return parameterPositions;
	}

	private void travelSubTrees(List<ITree> children, Map<String, List<Integer>> parameterPositions, Map<String, String> parameters) {
		for (ITree child : children) {
			if (child.getType() == 42) {//SimpleName;
				String simpleName = child.getLabel();
				if (parameters.containsKey(simpleName)) {
					List<Integer> positions = parameterPositions.get(simpleName);
					if (positions == null) {
						positions = new ArrayList<Integer>();
						positions.add(child.getPos());
						parameterPositions.put(simpleName, positions);
					} else {
						parameterPositions.get(simpleName).add(child.getPos());
					}
				}
	...
	@SuppressWarnings("unused")
	private int readStartPositionOfFirstStatement(ITree methodTree) {
		List<ITree> children = methodTree.getChildren();
		for (ITree child : children) {
