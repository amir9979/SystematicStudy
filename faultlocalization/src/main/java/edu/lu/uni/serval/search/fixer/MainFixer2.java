...
public class MainFixer2 {
	
	private static Logger log = LoggerFactory.getLogger(MainFixer2.class);
...
//	private TimeLine timeLine;
	...
	public MainFixer2(String path, String projectName, int bugId, String defects4jPath) {
		this.path = path;
	...
	public MainFixer2(String path, String metric, String projectName, int bugId, String defects4jPath) {
...
//			}
			...
			List<String> similarMethods = readSimilarMethods(index, dataTypeStr, methodInfoList);
			System.out.println(signature + " " + dataType + ": " + similarMethods.size());
			
			if (similarMethods.size() > 0) {
//				if (similarMethods.size() > 1000) continue;
				fixWithMatchedSimilarMethods(similarMethods, suspiciousMethodBodyCode, suspiciousClassName, javaBackup, classBackup, targetJavaFile,
						...
						errAssertBeforeFix, methodInfoList, methodName1, outputPath + dataType, signature);
			}
...
//		}
	...
	private void fixWithMatchedSimilarMethods(List<String> similarMethods, String suspiciousMethodBodyCode, String suspiciousClassName, File javaBackup, File classBackup, File targetJavaFile,
			File targetClassFile, int startLine, int endLine, DataPreparer dp,
			int errAssertBeforeFix, List<String> methodInfoList, String methodName, String type, String signature) {
...
//		packageName = packageName.substring(0, packageName.length() - 5);
		for (int i = 0, size = similarMethods.size(); i < size; i ++) {
			...
			if (!indexList.contains(Integer.valueOf(i))) continue;
...
//			}
            ...
            addCodeToFile(targetJavaFile, patch.patchMethodCode, startLine, endLine);// Insert the patch.
...
//					System.out.println(dateFormat.format(new Date()));
					...
					FileHelper.outputToFile(type + "/FixedBugs/" + buggyProject + "/buggyMethod_" + methodName + "_" + startLine + "_" + i + ".txt", 
							"//**********************************************************\n//  " + signature + 
							"\n//**********************************************************\n" + suspiciousMethodBodyCode, false);
					FileHelper.outputToFile( type + "/FixedBugs/" + buggyProject + "/patchMethod_" + methodName + "_" + startLine + "_" + i + ".txt",
							"//**********************************************************\n//  " + similarMethodInfo + 
...
//						System.out.println(dateFormat.format(new Date()));
						...
						FileHelper.outputToFile( type + "/PartialFixedBugs/" + buggyProject + "/buggyMethod_" + methodName + "_" + startLine + "_" + i + ".txt", 
								"//**********************************************************\n//  " + signature + 
								"\n//**********************************************************\n" + suspiciousMethodBodyCode, false);
						FileHelper.outputToFile( type + "/PartialFixedBugs/" + buggyProject + "/patchMethod_" + methodName + "_" + startLine + "_" + i + ".txt",
								"//**********************************************************\n//  " + similarMethodInfo + 
								"\n//**********************************************************\n" + patch.patchMethodCode, false);
					}
				}
	...
	private List<String> readSimilarMethods(int index, String type, List<String> methodInfoList) throws IOException {
	...
	private void addCodeToFile(File file, String patchMethodCode, int startLine, int endLine) {
