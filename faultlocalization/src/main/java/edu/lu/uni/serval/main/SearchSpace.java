...
public class SearchSpace implements Serializable {
	...
	public Map<String, List<SearchSpaceMethod>> searchSpace = new HashMap<>();
	...
	private void buildSearchSpace() throws IOException {
		...
		for (File returnTypeFile : returnTypeFiles) {
			if (returnTypeFile.isDirectory()) {
				...
				if (signaturesFile.exists()) {
					...
					List<SearchSpaceMethod> methods = new ArrayList<>();
					for (int index = 0; index < size; index ++) {
						SearchSpaceMethod method = new SearchSpaceMethod();
