...
public abstract class AbstractDebuggerLauncher {
    ...
    public void launch() {
        ...
        try {
            ...
            if (getProcess() != null) getProcess().destroy();
            debuggerFinished();
        }
    }
    ...
    protected abstract String testsToRunInString() throws Exception;
    ...
    protected void initTestResult(BreakpointEvent breakpointEvent) {
        ...
        ExpressionParser.GetFrame getFrame = getFrameGetter(breakpointEvent.thread(), 1, Tester.TEST_EXECUTION_METHOD_NAME);

        String testClass = "Error", testMethod = "Error";
        try {
            Value value = null;

            value = ExpressionParser.evaluate(Tester.TEST_REQUEST_VARIABLE_NAME + "." + TestRequest.TEST_CLASS_GETTER_NAME + "()", getVirtualMachine(), getFrame);
            ...
            value = ExpressionParser.evaluate(Tester.TEST_REQUEST_VARIABLE_NAME + "." + TestRequest.TEST_METHOD_GETTER_NAME + "()", getVirtualMachine(), getFrame);
    ...
    protected void updateTestResult(BreakpointEvent breakpointEvent) {
        ...
        try {
            ...
            value = ExpressionParser.evaluate(Tester.TEST_RESULT_VARIABLE_NAME + "." + TEST_RESULT_RUN_TIME_METHOD_NAME + "()", getVirtualMachine(), getFrame);
            if (value!=null)
                runTime = Long.valueOf(value.toString());

            value = ExpressionParser.evaluate(Tester.TEST_RESULT_VARIABLE_NAME + "." + TEST_RESULT_WAS_SUCCESSFUL_METHOD_NAME + "()", getVirtualMachine(), getFrame);
            if (value!=null)
...
//            throw new IllegalStateException();
    ...
    protected List<BreakpointRequest> addBreakPointToLocations(ReferenceType referenceType, Collection<LineLocation> locations, boolean shouldEnable) throws AbsentInformationException {
        List<BreakpointRequest> requests = new LinkedList<>();

        Method m = getMethodToMonitorFromType(referenceType);
        for (LineLocation location : locations) {
            // Create BreakpointEvent for desired locations
            List<Location> jdiLocations = m.locationsOfLine(location.getLineNo());
    ...
    protected Method getMethodToMonitorFromType(ReferenceType referenceType) {
        ...
        for (Method m : methods) {
            if ((qualifiedReturnTypeName.equals(m.returnTypeName()) ||
                    binaryReturnTypeName.equals(m.returnTypeName())) &&
                    sig2.equals(MethodUtil.getMethodSignature(m))) {
    ...
    private Map<String, Connector.Argument> prepareLaunchArguments(LaunchingConnector launchingConnector) throws Exception {
        ...
        options.setValue(getProject().commandLineStringForAgents() + " -ea -cp \"" + getProject().getClasspathForFixingStr() + "\"");
...
//        LoggingService.debug(event.toString());
        if (event instanceof VMStartEvent) {
            ...
            if (referenceType.name().equals(Tester.class.getName())) {
                setTestStartBreakpoint(registerTestBoundaryBreakpoint(referenceType, Tester.TesterBoundaryMethodName.TEST_START_METHOD_NAME).location());
                setTestEndBreakpoint(registerTestBoundaryBreakpoint(referenceType, Tester.TesterBoundaryMethodName.TEST_END_METHOD_NAME).location());
            }
    ...
    private BreakpointRequest registerTestBoundaryBreakpoint(ReferenceType referenceType, Tester.TesterBoundaryMethodName testerBoundaryMethodName) {
        String methodName = testerBoundaryMethodName.getMethodName();
    ...
    protected void setMtfEntryAndExitLocationBreakpoint(ReferenceType referenceType, List<BreakpointRequest> breakpointRequestList) {
        BreakpointRequest mtfEntryBreakPoint = registerTestBoundaryBreakpoint(referenceType, Tester.TesterBoundaryMethodName.START_METHOD_TO_MONITOR_METHOD_NAME);
        BreakpointRequest mtfExitBreakPoint = registerTestBoundaryBreakpoint(referenceType, Tester.TesterBoundaryMethodName.END_METHOD_TO_MONITOR_METHOD_NAME);
    ...
    protected void monitorExitState(BreakpointEvent breakpointEvent) {
        ExpressionParser.GetFrame getFrame = getFrameGetter(breakpointEvent.thread(), 1, null);
        ...
        getCurrentTestResult().addExitState(exitState.getExtendedStateWithoutErrorResult(getProject().getMethodToMonitor()
