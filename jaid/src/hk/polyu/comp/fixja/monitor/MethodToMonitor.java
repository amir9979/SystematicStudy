...
import hk.polyu.comp.fixja.ast.*;
...
public class MethodToMonitor {
    ...
    public void setAccessedFields(Set<String> accessedFields) {
        ...
        for (String field : accessedFields) {
            for (ExpressionToMonitor declaredField : getMethodDeclarationInfoCenter().getThisExpressionToMonitor().getFieldsToMonitor().values()) {
                if (field.equals(declaredField.getText())) {
