...
public class ExpressionCollector extends ASTVisitor {
    ...
    private boolean isSyntaxOK(Expression expNode){
        // Filter based on the expression form
        String text = expNode.toString();
        if (text.indexOf("\"") >= 0) return false;         // Expressions with double quote
        if (text.indexOf("++") >= 0 || text.indexOf("--") >= 0) return false;         // Expressions with ++ or --
    ...
    public boolean visit(InfixExpression node) {
        // fixme: normalize the expression to avoid duplications in further process.
        // For example, make null always appear first in a comparison with another expression, like in "null == exp" and "null != exp".
        // For operators satisfying the commutative law (e.g., +, &&, ||), sort the operands.
        categorizeExpression(node);
        return true;
    }
    ...
    public boolean visit(SimpleName node) {
        ...
        if (!shouldCheckType || binding instanceof IVariableBinding && !((IVariableBinding)binding).isField()) {
    ...
    public boolean visit(ThisExpression node) {
        categorizeExpression(node.getAST().newThisExpression());
        categorizeExpression(node);
        return false;
    }
