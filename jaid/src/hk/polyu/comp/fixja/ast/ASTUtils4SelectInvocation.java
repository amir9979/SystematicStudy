...
public class ASTUtils4SelectInvocation {
     ...
     * @return
     */
    public static List<String> assignableStateInvocation(boolean isNumberType, LineLocation location) {
            ...
            if (expType != null && !expType.isPrimitive()) {//找到当前location中的所有type 为reference的exp（包括方法调用）
                ...
                for (IMethodBinding method : validMethods) {
                    ...
                    if (method.getParameterTypes().length == 0) {
                        invocation = CommonUtils.appendInvoking(expToMonitor.getExpressionAST(), method.getName(), null).toString();
                        invocations.add(invocation);
                    } else {
                        Set<List<ASTNode>> paramsList = getCombinedParametersName(method, location);
                        for (List<ASTNode> params : paramsList) {
                            invocation = CommonUtils.appendInvoking(expToMonitor.getExpressionAST(), method.getName(), params).toString();
                            invocations.add(invocation);
                        }
                    }
                }
