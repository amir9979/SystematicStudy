...
public class Strategy4IfCondition extends Strategy {
     ...
     * 新条件：如果OldIfCon 的某个sub_condition 中含有snapshotEXP的全部subEXP，则将此sub_condition替换为T/F
     *
     * @param oldIfStmt
     */
    ...
    private void disableSubIfCon(IfStatement oldIfStmt, Expression oldSubCon) {
        ...
        for (ExpressionToMonitor expressionToMonitor : getStateSnapshot().getSnapshotExpression().getOperands()) {
            Expression toDisable = (Expression) findNodeByExp.find(expressionToMonitor.getExpressionAST());
