...
//	private String dependjpath;
	...
	public PatchGenerator(String bug_id, String ssfix_dpath, String proj_dpath, String proj_testbuild_dpath,
//			String dependjpath, 
			String tsuitefpath, String failed_testcases, boolean delete_failed_patch,
			int parallelgranularity) {
		this.bug_id = bug_id;
...
//		this.dependjpath = dependjpath;
		...
		if (parallel_granularity <= 0) {
			parallel_granularity = 1;
		} else if (parallel_granularity >= 1024) {
			parallel_granularity = 1024;
		}
		pgen0 = new PatchGenerator0();
		pval = new PatchValidator(bug_id, proj_testbuild_dpath, tsuite_fpath, ssfix_dpath);
		tested_pset = new HashSet<String>();
	}

	public Patch generatePatch(BuggyChunk bchunk, CandidateChunk cchunk, String fix_dpath, File javaFile, File classFile) {
		return generatePatch(bchunk, cchunk, fix_dpath, false, false, false, javaFile, classFile);
	}

	public Patch generatePatch(BuggyChunk bchunk, CandidateChunk cchunk, String fix_dpath, boolean print_renaming,
			boolean print_ccmatching, boolean print_patches, File javaFile, File classFile) {

		// Generate all candidate patches
		System.err.println("Generating Initial Patches ...");
		List<Modification> mod_list = pgen0.generatePatches(bchunk, cchunk, print_renaming, print_ccmatching,
				print_patches);
		if (mod_list.isEmpty()) {
...
//			rslt_patch0 = validatePatches0(patch_text_list, fix_dpath, bfpath, bfname, javaFile, classFile);
//		} else {
//			rslt_patch0 = validatePatches1(patch_text_list, fix_dpath, bfpath, bfname, javaFile, classFile);
//		}
		rslt_patch0 = validatePatches0(patch_text_list, fix_dpath, bfpath, bfname, javaFile, classFile);
	...
	private Patch validatePatches0(List<String> patch_text_list, String fix_dpath, String bfpath, String bfname, File javaFile, File classFile) {
		...
		for (int i = 0; i < patch_text_list_size; i++) {
			...
			Patch patch = pval.validate(patch_text_list.get(i), patch_fpath, patch_dpath, failed_testcases, javaFile, classFile);
			if (patch != null && patch.isCorrect()) {
				...
				if (delete_failed_patch) {
					try {
						FileUtils.deleteDirectory(patch_d);
					} catch (IOException e) {
	...
	@SuppressWarnings("unused")
	private Patch validatePatches1(List<String> patch_text_list, String fix_dpath, String bfpath, String bfname, File javaFile, File classFile) {
		...
		if (patch_text_list_size < para_gran) {
			Patch patch = validatePatches1Helper(patch_text_list, 0, patch_text_list_size, fix_dpath, bfpath, bfname, javaFile, classFile);
			...
			for (int i = 0; i < patch_text_list_size;) {
				...
				if (i + para_gran <= patch_text_list_size) {
					patch = validatePatches1Helper(patch_text_list, i, i + para_gran, fix_dpath, bfpath, bfname, javaFile, classFile);
					if (patch.isCorrect()) {
						patch.setTestedNum((i + 1) * para_gran);
						return patch;
					}
				} else {
					patch = validatePatches1Helper(patch_text_list, i, patch_text_list_size, fix_dpath, bfpath, bfname, javaFile, classFile);
					if (patch.isCorrect()) {
						patch.setTestedNum(patch_text_list_size);
						return patch;
					}
	...
	private Patch validatePatches1Helper(List<String> patch_text_list, int start, int end, String fix_dpath,
			String bfpath, String bfname, File javaFile, File classFile) {
		...
		for (int i = start; i < end; i++) {
			...
			String patch_fpath = patch_dpath + "/" + bfname;
			ValidatorRunner vrunner = new ValidatorRunner(patch_text_list.get(i), patch_fpath, patch_dpath,
					failed_testcases, javaFile, classFile);
		...
		try {
			patch_future_list = exe_service.invokeAll(call_list);
		} catch (Throwable t) {
		...
		if (patch_future_list == null) {
			return new Patch(null, false);
		}
		for (Future<Patch> patch_future : patch_future_list) {
			Patch patch = null;
			try {
				patch = patch_future.get();
			} catch (Throwable t) {
	...
	private class ValidatorRunner implements Callable<Patch> {
		...
		public ValidatorRunner(String patch_text, String patch_fpath, String patch_dpath, String failed_testcases, File javaFile, File classFile) {
		...
		public Patch call() {
			Patch patch = pval.validate(patch_text, patch_fpath, patch_dpath, failed_testcases, javaFile, classFile);
			if (!patch.isCorrect() && delete_failed_patch) {
				File patch_f = new File(patch_fpath);
				try {
					FileUtils.deleteDirectory(patch_f.getParentFile());
				} catch (Throwable t) {
					System.err.println(t);
					t.printStackTrace();
				}
