...
import edu.brown.cs.ssfix.patchgen.BuggyChunk;
import edu.brown.cs.ssfix.patchgen.CandidateChunk;
import edu.brown.cs.ssfix.patchgen.Patch;
import edu.brown.cs.ssfix.patchgen.PatchGenerator;
import edu.brown.cs.ssfix.search.SearchStringGenerator;
import edu.brown.cs.ssfix.util.ASTNodeFinder;
import edu.brown.cs.ssfix.util.ASTNodeLoader;
import edu.brown.cs.ssfix.util.CandidateLoader;
...
//	private String dependjpath;
	...
	public Repair0(String bugid, String ssfixdpath, String cockerdpath, String projdpath, String projtestbuilddpath,
//			String dependjpath, 
			String tsuitefpath, String failedtestcases, String analysismethod, int max_candidates,
			int paragran, boolean usesearchcache, boolean useextendedcodebase, boolean deletefailedpatches) {
		bug_id = bugid;
...
//		this.dependjpath = dependjpath;
		...
		// localproj_merobase_pathptn =
		// Pattern.compile(".+/defects4j-bugs/.+/.+_buggy/.+\\.java|.+/merobase_split_100/.+/srcs/.+\\.java|.+/.+_local_fix\\.java");
		localproj_merobase_pathptn = Pattern
				.compile(".+/defects4j-bugs/.+/.+_buggy/.+\\.java|.+/merobase_split_100/.+/srcs/.+\\.java");
	...
	private Patch repair0(String bfpath, String bstmt_loc, String fix_dpath) {
		...
		String bfctnt = null;
		try {
			bfctnt = FileUtils.readFileToString(bf, (String) null);
		} catch (IOException e) {
		...
		File cocker_rslt_f = new File(cocker_rslt_fpath);
		try {
			cchunk_lines = FileUtils.readLines(cocker_rslt_f, (String) null);
		} catch (Throwable t) {
		...
		PatchGenerator pgen = new PatchGenerator(bug_id, ssfix_dpath, proj_dpath, proj_testbuild_dpath, //dependjpath,
				tsuite_fpath, failed_testcases, delete_failed_patches, parallel_granularity);
		...
		for (int i = 0; i < cchunk_num; i++) {
			...
			if (cchunk_count >= max_candidate_num) {
				break;
			}

			// Generate the candidate chunk
			System.out.println("Looking at Candidate Chunk " + (i + 1));
			...
			if (cfpath.startsWith("file://")) {
				cfpath = cfpath.substring(7);
			}

			if (localproj_merobase_only) {
				if (cfpath.endsWith("closure_14_fix.java") || cfpath.endsWith("math_33_local_fix.java")
						|| cfpath.endsWith("math_53_local_fix.java") || cfpath.endsWith("math_59_local_fix.java")
...
//			File javaFile = new File(bfpath);
			String classPath = Global.dp.classPath;
			String srcPath = Global.dp.srcPath;
			File classFile = new File(bfpath.replace(srcPath, classPath).replace(".java", ".class"));
			File srcFileBackup = new File(bfpath + ".backup");
			File classFileBackup = new File(bfpath.replace(srcPath, classPath).replace(".java", ".class.backup"));
			try {
				FileUtils.writeStringToFile(srcFileBackup, bfctnt);
				if (classFile.exists()) {
					String classContent = FileUtils.readFileToString(classFile, (String) null);
					FileUtils.writeStringToFile(classFileBackup, classContent);
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
			
			Patch patch = pgen.generatePatch(bchunk, cchunk, fix_dpath0, bf, classFile);
			if (patch == null) {
			...
			try {
				FileUtils.writeStringToFile(bf, bfctnt);
				if (classFileBackup.exists()) {
					String classContent = FileUtils.readFileToString(classFileBackup, (String) null);
					FileUtils.writeStringToFile(classFile, classContent);
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (rslt_patch == null) {
			rslt_patch = new Patch(null, false);
		}
	...
	private Patch repair1(String bfpath, String bstmt_loc, String fix_dpath) {
		...
		String cocker_rslt_fpath = fix_dpath + "/cocker_rslt";
//		List<String> cchunk_lines = search_invoker.invoke(bfpath, search_loc, ssfix_dpath, anal_method,
//				cocker_rslt_fpath, false); // no filtering is performed
//		if (cchunk_lines == null || cchunk_lines.isEmpty()) {
...
//		}
		...
		PatchGenerator pgen = new PatchGenerator(bug_id, ssfix_dpath, proj_dpath, proj_testbuild_dpath, 
				tsuite_fpath, failed_testcases, delete_failed_patches, parallel_granularity);
		String bloc = getChunkLoc(bfpath, bfctnt, search_loc);
		if (bloc == null || "".equals(bloc)) {
			System.err.println("Cannot get the correct chunk loc from " + search_loc);
...
//			File javaFile = bf;bfpath
	 ...
	 * ASTNodeLoader.getResolvedASTNode(fpath, fctnt); return getChunkLoc0(cu, loc);
	 * }
	 */
	...
	/*
	 * If loc corresponds to a method, get the loc for body stmts. flag 0: bchunk
	 * flag 1: cchunk
	 */
	/*
	 * private String getChunkLoc0(String fpath, String loc, int flag, String
	 * analmethod) { List<ASTNode> found_nodes = null; CompilationUnit cu = null; if
	 * (flag == 0) { cu = (CompilationUnit) ASTNodeLoader.getASTNode(new
	 * File(fpath)); found_nodes = ASTNodeFinder.find(cu, loc); } else { String
	 * cfctnt = CandidateLoader.getFileContent(fpath, analmethod); cu =
	 * (CompilationUnit) ASTNodeLoader.getResolvedASTNode(fpath, cfctnt);
	 * found_nodes = ASTNodeFinder.find(cu, loc); }
	 * 
	 * if (found_nodes.size()==1) { ASTNode found_node = found_nodes.get(0); if
	 * (found_node instanceof MethodDeclaration) { MethodDeclaration md =
	 * (MethodDeclaration) found_node; Block md_body = md.getBody(); if (md_body ==
	 * null) { return null; } //no-body List stmt_obj_list =
	 * md.getBody().statements(); String new_loc = null; for (Object stmt_obj :
	 * stmt_obj_list) { ASTNode stmt = (ASTNode) stmt_obj; int pos =
	 * stmt.getStartPosition(); String new_subloc =
	 * "slc:"+cu.getLineNumber(pos)+","+cu.getColumnNumber(pos); if (new_loc ==
	 * null) { new_loc = new_subloc; } else { new_loc += ";" + new_subloc; } }
	 * return new_loc; } }
	 * 
	 * return loc; }
	 */
	...
	private String[] getClassNameAndMethodSignature(ASTNode tnode) {
		...
		if (atd != null) {
			...
			if (pd != null) {
				cname = pd.getName().toString() + "." + cname;
			}
		}
	...
	private String getMethodSignatureString(MethodDeclaration md) {
		...
		for (Object param_obj : param_list) {
			...
			if (marg == null) {
				marg = param_svd.getType().toString();
			} else {
				marg += "$" + param_svd.getType().toString();
			}
		}
