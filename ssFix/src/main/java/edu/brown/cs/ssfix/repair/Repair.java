...
//		String dependjpath = Global.dependjpath;
		...
		if (!fauloc_f.exists()) {
			...
			try {
				fauloc_lines = FileUtils.readLines(fauloc_f, (String) null);
			} catch (Throwable t) {
		...
		if (failed_testcases == null) {
			...
			for (String fauloc_line : fauloc_lines) {
				...
				if (fauloc_line0.startsWith("Test failed:")) {
					...
					String failed_testcase = ((index1 == -1) ? fauloc_line0.substring(index0)
							: fauloc_line0.substring(index0, index1)).trim();
					if (failed_testcases == null) {
				...
				if (fauloc_line0.startsWith("Suspicious line:")) {
					break;
				}
			}
		...
		Repair0 repair0 = new Repair0(bug_id, ssfix_dpath, cocker_dpath, proj_dpath, proj_testbuild_dpath,// dependjpath,
				tsuite_fpath, failed_testcases, anal_method, max_candidates, parallel_granularity, use_search_cache,
				use_extended_codebase, delete_failed_patches);
