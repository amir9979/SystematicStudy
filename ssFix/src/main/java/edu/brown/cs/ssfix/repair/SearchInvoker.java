...
import edu.brown.cs.ssfix.search.SearchFilter;
...
public class SearchInvoker {
	...
	public List<String> invoke(String bfpath, String search_loc, String ssfix_dpath, String anal_method,
			String cocker_rslt_fpath, boolean filter) {
		...
		for (String cocker_arg : cocker_args) {
			System.out.print(cocker_arg + (cocker_arg.startsWith("-") ? " " : "\n"));
		}
		...
		try {
			search_rslt0 = future.get(30l, TimeUnit.MINUTES);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (!filter) {
			if (cocker_rslt_fpath != null) {
				...
				try {
					FileUtils.writeStringToFile(cocker_rslt_f, search_rslt_str);
				} catch (Throwable t) {
	...
	/*
	 * All the located nodes should be from only ONE method. flag 0: bchunk flag 1:
	 * cchunk
	 */
