...
class Replacer {
	/*
	 * NOTE: node1 & node2 should both be NORMALIZED (block inserted for a non-block
	 * body). NOTE: node2 MUST be a node copy (as an un-rooted node).
	 */
	public List<Modification> replaceWith(ASTNode node1, ASTNode node2, AST ast1) {
		...
		if (isEquivalent(node1, node2)) {
			return rw_list;
		}

		ASTRewrite rw0 = ASTRewrite.create(ast1);
		rw0.replace(node1, node2, null);
		Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(node1, node2, 0),
				ModificationSizeCalculator.calculate0(node1, node2, 1));
		rw_list.add(mod0);
		...
		if (node_type1 == node_type2) {
			if (node_type1 == ASTNode.CATCH_CLAUSE) {
				...
				for (Modification rw_tmp : rw_list_tmp) {
					rw_list.add(rw_tmp);
				}
			} else if (node_type1 == ASTNode.CONDITIONAL_EXPRESSION) {
				List<Modification> rw_list_tmp = replaceWith((ConditionalExpression) node1,
						(ConditionalExpression) node2, ast1);
				for (Modification rw_tmp : rw_list_tmp) {
					rw_list.add(rw_tmp);
				}
			} else if (node_type1 == ASTNode.CONSTRUCTOR_INVOCATION) {
				List<Modification> rw_list_tmp = replaceWith((ConstructorInvocation) node1,
						(ConstructorInvocation) node2, ast1);
				for (Modification rw_tmp : rw_list_tmp) {
					rw_list.add(rw_tmp);
				}
				...
				List<Modification> rw_list_tmp = replaceWith((SuperConstructorInvocation) node1,
						(SuperConstructorInvocation) node2, ast1);
				for (Modification rw_tmp : rw_list_tmp) {
					rw_list.add(rw_tmp);
				}
				...
				List<Modification> rw_list_tmp = replaceWith((SuperMethodInvocation) node1,
						(SuperMethodInvocation) node2, ast1);
				for (Modification rw_tmp : rw_list_tmp) {
					rw_list.add(rw_tmp);
				}
			} else if (node_type1 == ASTNode.POSTFIX_EXPRESSION) {
				List<Modification> rw_list_tmp = replaceWith((PostfixExpression) node1, (PostfixExpression) node2,
						ast1);
				for (Modification rw_tmp : rw_list_tmp) {
					rw_list.add(rw_tmp);
				}
				...
				List<Modification> rw_list_tmp = replaceWith((SynchronizedStatement) node1,
						(SynchronizedStatement) node2, ast1);
				for (Modification rw_tmp : rw_list_tmp) {
					rw_list.add(rw_tmp);
				}
	...
	private List<Modification> replaceWith(ConditionalExpression ce1, ConditionalExpression ce2, AST tast) {
		...
		if (!isEquivalent(c1, c2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(c1, c2, 0),
					ModificationSizeCalculator.calculate0(c1, c2, 1));
			mod_list.add(mod0);
		}
		...
		if (!isEquivalent(t1, t2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(t1, t2, 0),
					ModificationSizeCalculator.calculate0(t1, t2, 1));
			mod_list.add(mod0);
		}
		...
		if (!isEquivalent(e1, e2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(e1, e2, 0),
					ModificationSizeCalculator.calculate0(e1, e2, 1));
			mod_list.add(mod0);
		}
	...
	private List<Modification> replaceWith(SuperConstructorInvocation sci1, SuperConstructorInvocation sci2, AST tast) {
		...
		if (!isEquivalent(exp1, exp2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(exp1, exp2, 0),
					ModificationSizeCalculator.calculate0(exp1, exp2, 1));
			mod_list.add(mod0);
		}
		return mod_list;
	}
	...
	private List<Modification> replaceWith(Assignment a1, Assignment a2, AST tast) {
		...
		if (!isEquivalent(lhs1, lhs2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(lhs1, lhs2, 0),
					ModificationSizeCalculator.calculate0(lhs1, lhs2, 1));
			mod_list.add(mod0);
		}
		...
		if (!op1.toString().equals(op2.toString())) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate1(op1, op2, 0),
					ModificationSizeCalculator.calculate1(op1, op2, 1));
			mod_list.add(mod0);
		}
		...
		if (!isEquivalent(rhs1, rhs2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(rhs1, rhs2, 0),
					ModificationSizeCalculator.calculate0(rhs1, rhs2, 1));
			mod_list.add(mod0);
		}
		return mod_list;
	}
		...
		if (hasExOp1 == hasExOp2) {
			...
			if (exOps_size1 == exOps_size2) {
				...
				if (!isEquivalent(lhs1, lhs2)) {
					...
					Modification mod0 = new Modification(rw0, "REPLACE",
							ModificationSizeCalculator.calculate0(lhs1, lhs2, 0),
							ModificationSizeCalculator.calculate0(lhs1, lhs2, 1));
					mod_list.add(mod0);
				}
				...
				if (!opt1.toString().equals(opt2.toString())) {
					...
					Modification mod0 = new Modification(rw0, "REPLACE",
							ModificationSizeCalculator.calculate1(opt1, opt2, 0),
							ModificationSizeCalculator.calculate1(opt1, opt2, 1));
					mod_list.add(mod0);
				}
				...
				if (!isEquivalent(rhs1, rhs2)) {
					...
					Modification mod0 = new Modification(rw0, "REPLACE",
							ModificationSizeCalculator.calculate0(rhs1, rhs2, 0),
							ModificationSizeCalculator.calculate0(rhs1, rhs2, 1));
					mod_list.add(mod0);
				}
				...
				if (exOps_size1 > 0) {
					for (int i = 0; i < exOps_size1; i++) {
						...
						if (!isEquivalent(op1, op2)) {
							...
							Modification mod0 = new Modification(rw0, "REPLACE",
									ModificationSizeCalculator.calculate0(op1, op2, 0),
									ModificationSizeCalculator.calculate0(op1, op2, 1));
							mod_list.add(mod0);
						}
					}
				}
	...
	private List<Modification> replaceWith(MethodInvocation mi1, MethodInvocation mi2, AST tast) {
		...
		if (!isEquivalent(sname1, sname2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE",
					ModificationSizeCalculator.calculate0(sname1, sname2, 0),
					ModificationSizeCalculator.calculate0(sname1, sname2, 1));
			mod_list.add(mod0);
		}
	...
	private List<Modification> replaceWith(SuperMethodInvocation smi1, SuperMethodInvocation smi2, AST tast) {
		...
		if (!isEquivalent(q1, q2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(q1, q2, 0),
					ModificationSizeCalculator.calculate0(q1, q2, 1));
			mod_list.add(mod0);
		}
		return mod_list;
	}
	...
	private List<Modification> replaceWith(PostfixExpression pe1, PostfixExpression pe2, AST tast) {
		...
		if (!isEquivalent(op1, op2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(op1, op2, 0),
					ModificationSizeCalculator.calculate0(op1, op2, 1));
			mod_list.add(mod0);
		}
		...
		if (!opt1.toString().equals(opt2.toString())) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate1(opt1, opt2, 0),
					ModificationSizeCalculator.calculate1(opt1, opt2, 1));
			mod_list.add(mod0);
		}
		return mod_list;
	...
	private List<Modification> replaceWith(CatchClause cc1, CatchClause cc2, AST tast) {
		...
		if (!isEquivalent(b1, b2)) {
			...
			Modification mod1 = new Modification(rw1, "REPLACE", ModificationSizeCalculator.calculate0(b1, b2, 0),
					ModificationSizeCalculator.calculate0(b1, b2, 1));
			mod_list.add(mod1);
		}
		return mod_list;
	}
	...
	private List<Modification> replaceWith(ReturnStatement rs1, ReturnStatement rs2, AST tast) {
		...
		if (rtype1 != null && rtype2 != null) {
			...
			if (("boolean".equals(rtype1_str) || "Boolean".equals(rtype1_str))
				...
				Modification mod1 = new Modification(rw1, "REPLACE",
						ModificationSizeCalculator.calculate0(e1, comb0, 0),
						ModificationSizeCalculator.calculate0(e1, comb0, 1));
				mod_list.add(mod1);
				rw2.set(rs2, ReturnStatement.EXPRESSION_PROPERTY, comb1, null);
				Modification mod2 = new Modification(rw2, "REPLACE",
						ModificationSizeCalculator.calculate0(e1, comb1, 0),
						ModificationSizeCalculator.calculate0(e1, comb1, 1));
				mod_list.add(mod2);
			}
		}
		return mod_list;
	...
	private List<Modification> replaceWith(Block block1, Block block2, AST tast) {
		...
		Modification mod = new Modification(rw, "REPLACE", ModificationSizeCalculator.calculate0(block1, block2, 0),
				ModificationSizeCalculator.calculate0(block1, block2, 1));
		mod_list.add(mod);
		return mod_list;
	}
	...
	private Modification replaceWithForInitializers(ASTRewrite rw2, ForStatement forstmt1, ForStatement forstmt2,
			AST tast) {
		...
		if (!isEquivalent(init_list1, init_list2)) {
			...
			mod2 = new Modification(rw2, "REPLACE", ModificationSizeCalculator.calculate2(init_list1, init_list2, 0),
					ModificationSizeCalculator.calculate2(init_list1, init_list2, 1));
		}
		return mod2;
	}

	/* Return Value could be NULL. */
	private Modification replaceWithForUpdaters(ASTRewrite rw3, ForStatement forstmt1, ForStatement forstmt2,
			AST tast) {
		...
		if (!isEquivalent(upt_list1, upt_list2)) {
			...
			mod3 = new Modification(rw3, "REPLACE", ModificationSizeCalculator.calculate2(upt_list1, upt_list2, 0),
					ModificationSizeCalculator.calculate2(upt_list1, upt_list2, 1));
		}
		return mod3;
	}
	...
	private Modification replaceWithForHeader(ForStatement forstmt1, ForStatement forstmt2, AST tast) {
		...
		if (!isEquivalent(c1, c2) && (c2 != null)) {
			...
			mod = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(c1, c2, 0),
					ModificationSizeCalculator.calculate0(c1, c2, 1));
		}
	...
	private List<Modification> replaceWith(IfStatement ifstmt1, IfStatement ifstmt2, AST tast) {
		...
		if (!isEquivalent(cond1, cond2)) {
			...
			Modification mod0 = new Modification(rw0, "REPLACE", ModificationSizeCalculator.calculate0(cond1, cond2, 0),
					ModificationSizeCalculator.calculate0(cond1, cond2, 1));
			mod_list.add(mod0);
			...
			Modification mod1 = new Modification(rw1, "REPLACE", ModificationSizeCalculator.calculate0(cond1, comb0, 0),
					ModificationSizeCalculator.calculate0(cond1, comb0, 1));
			mod_list.add(mod1);
			rw2.set(ifstmt1, IfStatement.EXPRESSION_PROPERTY, comb1, null);
			Modification mod2 = new Modification(rw2, "REPLACE", ModificationSizeCalculator.calculate0(cond1, comb1, 0),
					ModificationSizeCalculator.calculate0(cond1, comb1, 1));
			mod_list.add(mod2);
		}
		...
		if (!isEquivalent(else1, else2)) {
			if (else1 == null || else2 == null) {
				...
				Modification mod2 = new Modification(rw2, "REPLACE",
						ModificationSizeCalculator.calculate0(else1, else2, 0),
						ModificationSizeCalculator.calculate0(else1, else2, 1));
				mod_list.add(mod2);
			} else {
	...
	private List<Modification> replaceWith(SwitchStatement ss1, SwitchStatement ss2, AST tast) {
		...
		if (!isEquivalent(body1, body2)) {
			...
			Modification mod1 = new Modification(rw1, "REPLACE", ModificationSizeCalculator.calculate2(body1, body2, 0),
					ModificationSizeCalculator.calculate2(body1, body2, 1));
			mod_list.add(mod1);
		}
	...
	private List<Modification> replaceWith(TryStatement try_stmt1, TryStatement try_stmt2, AST tast) {
		...
		/*
		 * //Ignored temporarily. List catch_list1 = try_stmt1.catchClauses(); List
		 * catch_list2 = try_stmt2.catchClauses(); if (!isEquivalent(catch_list1,
		 * catch_list2)) { ASTRewrite rw1 = ASTRewrite.create(tast); rw1.set(try_stmt1,
		 * TryStatement.CATCH_CLAUSES_PROPERTY, catch_list2, null); mod_list.add(rw1); }
		 */
		...
		if (size1 == size2) {
			for (int i = 0; i < size1; i++) {
				...
				if (!isEquivalent(arg1, arg2)) {
					...
					Modification mod0 = new Modification(rw0, "REPLACE",
							ModificationSizeCalculator.calculate0(arg1, arg2, 0),
							ModificationSizeCalculator.calculate0(arg1, arg2, 1));
					mod_list.add(mod0);
				}
			}
		}
