...
import edu.brown.cs.ssfix.util.CommandExecutor;
...
public class Main {
	...
	public static void main1(String[] args) {
		...
		try {
			cmd_line = clparser.parse(options, args);
		...
		String bugid = null, outputdpath = null, projdpath = null, projsrcdpath = null, ssfixdpath = null,
				projtestbuilddpath = null, tsuitefpath = null;
...
//			Global.dependjpath = value;
		...
		if (cmd_line.hasOption("maxfaultylines")) {
			...
			System.out.println(
					"The Maximum Number of Faulty Lines to be Considered for Repair: " + Global.maxfaultylines);
		}
		 ...
		 * System.out.println("Run the Repair in Parallel " + true); Global.runparallel
		 * = true; } else { System.out.println("Run the Repair in Parallel " +
		 * Global.runparallel); }
		 */
		...
		if (!output_bugid_dir.exists()) {
			output_bugid_dir.mkdir();
		}
		...
		if (cmd_line.hasOption("tsuitefpath")) {
			...
			// Generate a test suite file including all the test classes in the binary test
			// directory
			tsuitefpath = output_bugid_dpath + "/testsuite_classes";
			File tsuitef = new File(tsuitefpath);
			if (!tsuitef.exists()) {
				String[] path_convert_cmds = new String[] { "ant", "-f", "pathconverter.xml",
						"-Dtestbuilddir=" + projtestbuilddpath, "-Doutputdir=" + output_bugid_dpath,
						"-Doutputfname=testsuite_classes", "write_tsuitefile" };
	...
	public static void main(String[] args) {
		String outputdpath = "output/";
		
		/*
		 * Chart: 1, 20, 24, 
		 * Closure: 14, 115
		 * Lang: 6, 21, 33, 43, 59,
		 * Math: 30, 33, 41, 50, 53, 57, 59, 70, 79, 80, 
		 */
		String bugid = "Chart_1";
		String projdpath = "/Users/kui.liu/Public/Defects4JDataBackup/" + bugid;
		List<String> paths = PathUtils.getSrcPath(bugid);
		String projsrcdpath = projdpath + paths.get(2);
		String ssfixdpath = "/Users/kui.liu/eclipse-workspace/ssFix";;
		String projtestbuilddpath = projdpath + paths.get(1);
		String tsuitefpath = "testCases/" + bugid + ".txt";
		...
		String[] testCases = dp.testCases;
		for (String testCase : testCases) {
			cases.append(testCase).append(";");
		}
		try {
			FileUtils.writeStringToFile(new File(tsuitefpath), cases.toString());
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		Global.bugid = bugid;
		Global.dp = dp;
		Global.defects4jPath = "/Users/kui.liu/Public/git/defects4j/";
//		Global.dependjpath = ""; 
		Global.projdpath = projdpath;
		Global.projsrcdpath = projsrcdpath;
		Global.projbuilddpath = projdpath + paths.get(0);
		Global.projtestbuilddpath = projtestbuilddpath;
		Global.failedtestcases = "";//TODO
		Global.outputdpath = outputdpath;
//		Global.cockerdpath = "Cocker Directory Path";
		Global.ssfixdpath = ssfixdpath;
//		Global.maxfaultylines = Integer.parseInt(value);
//		Global.maxcandidates = Integer.parseInt(value);
//		Global.parallelgranularity = Integer.parseInt(value);
//		Global.analysismethod = value;
		Global.faulocfpath = outputdpath + bugid + "/gzoltar_fauloc";
		Global.usesearchcache = true;
		Global.useextendedcodebase = true;
		Global.deletefailedpatches = true;
