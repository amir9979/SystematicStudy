...
import edu.brown.cs.ssfix.util.ASTNodeTracker;
import edu.brown.cs.ssfix.util.TrackLoc;
...
public class PatchGenerator0 {
	...
	public List<Modification> generatePatches(BuggyChunk bchunk, CandidateChunk cchunk, boolean print_renaming,
			boolean print_ccmatching, boolean print_patches) {
		...
		if (!RENAME_LOCAL_CHUNK && cchunk.isLocal()) {
			...
			rcfctnts = renamer.rename(bchunk.getFilePath(), bchunk.getLoc(), bchunk.getFileContent(),
					cchunk.getFilePath(), cchunk.getLoc(), cchunk.getFileContent(), print_renaming);
		}
