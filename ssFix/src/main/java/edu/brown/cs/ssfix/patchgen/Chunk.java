...
public class Chunk {
	...
	/*
	 * public Chunk(String fpath, String loc) { this.fpath = fpath; this.loc = loc;
	 * this.node_list = loadNodes(); this.is_normalized = false; }
	 */
	...
	/*
	 * private List<ASTNode> loadNodes() { CompilationUnit cu = null; try { cu =
	 * (CompilationUnit) ASTNodeLoader.getResolvedASTNode(new File(fpath)); } catch
	 * (Throwable t) { System.err.println(t); t.printStackTrace(); } if (cu == null)
	 * { return new ArrayList<ASTNode>(); } else { return ASTNodeFinder.find(cu,
	 * loc); } }
	 */
